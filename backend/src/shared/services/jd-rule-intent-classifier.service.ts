...
INFORMATIONAL - Company benefits, culture, perks, company description (NOT matchable against CV)
Examples: "Health insurance", "Flexible hours", "Fast-paced startup", "We are a leading fintech"
...
Return ONLY the category name: REQUIREMENT, RESPONSIBILITY, QUALIFICATION, INFORMATIONAL, or PREFERENCE`
    ...
    private buildBatchPrompt(rules: JdRuleForClassification[]): string {
        const rulesList = rules
            .map((rule, idx) => `${idx + 1}. "${rule.content}"`)
            .join('\n')
...
[
  ...
  ...
    ...
    private parseBatchResponse(
        responseText: string,
        rules: JdRuleForClassification[],
    ): Map<string, RuleIntent> {
        const results = new Map<string, RuleIntent>()
